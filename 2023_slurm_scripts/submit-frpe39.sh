#!/bin/bash

#SBATCH -t 76:00:0                              #Time for the job to run
#SBATCH --job-name=IndexVCF_Subset     #Name of the job

#SBATCH -n 1                                    #Number of cores needed for the job; cannot be more than the number of cores available on a single node
#SBATCH --nodes=1

#SBATCH --partition=normal                      #Name of the queue

#SBATCH --mail-type ALL                         #Send email on start/end
#SBATCH --mail-user frpe222@uky.edu             #Where to send email

#SBATCH --account=coa_nmte222_uksr              #Name of account to run under

echo "Job $SLURM_JOB_ID running on SLURM NODELIST: $SLURM_NODELIST "

# Define input and output directories
input_dir=/scratch/frpe222/Sorted
output_dir=/scratch/frpe222/Variant


#little draft
singularity run --app bcftools113 /share/singularity/images/ccs/ngstools/samtools-1.13+matplotlib-bcftoools-1.13.sinf \
  bcftools view -Oz -o compressedP6_var.flt.vcf.gz P6_var.flt.vcf

singularity run --app bcftools113 /share/singularity/images/ccs/ngstools/samtools-1.13+matplotlib-bcftoools-1.13.sinf \
  bcftools index compressedP6_var.flt.vcf.gz

singularity run --app bcftools113 /share/singularity/images/ccs/ngstools/samtools-1.13+matplotlib-bcftoools-1.13.sinf \
  bcftools view -H -r 3L:18164174-18189228 compressedP6_var.flt.vcf.gz > P6_hid_var.txt

# for i in A1a; do #A2a B1a B2a C1a C2a D1a D2a; do
#   # Run commands
#   #singularity run --app bcftools113 /share/singularity/images/ccs/ngstools/samtools-1.13+matplotlib-bcftoools-1.13.sinf \
#   #bcftools mpileup -Ou -f /scratch/frpe222/Genome/dmel-short-header.fa "${input_dir}"/${i}_sorted.bam | \
#   singularity run --app bcftools113 /share/singularity/images/ccs/ngstools/samtools-1.13+matplotlib-bcftoools-1.13.sinf \
#   bcftools call -Ou -mv | \
#   singularity run --app bcftools113 /share/singularity/images/ccs/ngstools/samtools-1.13+matplotlib-bcftoools-1.13.sinf \
#   bcftools filter -s LowQual -e '%QUAL<20 || DP>100' > "${output_dir}"/${i}_var.flt.vcf
# done